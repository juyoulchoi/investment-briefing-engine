...
public class ReferenceDataAdminController {
 ...
 private StockRow row(TbStk x){return new StockRow(x.getStockId(),x.getStockCode(),x.getTicker(),x.getStockName(),x.getStockEnglishName(),x.getMarketCode(),x.getCountryCode(),x.getCurrencyCode(),x.getAssetType(),x.getStockGrade(),x.getBaseIndex()==null?null:x.getBaseIndex().getIndexId(),x.getSectorCode(),x.getSectorName(),x.getIndustryCode(),x.getIndustryName(),x.getRegularBuyYn(),x.getAdditionalBuyYn(),x.getRebuyYn(),x.getUseYn());}
