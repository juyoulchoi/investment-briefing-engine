...
public class OperationalDataAdminController {
 ...
 public record HoldingRow(Long holdingId,Long accountId,AccountType accountType,Long stockId,String stockCode,String stockName,BigDecimal holdingQuantity,BigDecimal averagePrice,BigDecimal currentPrice,BigDecimal exchangeRate,BigDecimal evaluationAmount,BigDecimal profitLossRate,BigDecimal targetWeight,BigDecimal currentWeight,HoldingStatus holdingStatus,RegularBuyStatus buyStatus,String userPauseYn,String memo,String useYn){}
 public record RegularBuyRequest(@NotNull Long accountId,@NotNull Long stockId,@Min(1) Integer priority,@Size(max=50) String investmentGrade,@Size(max=1000) String memo,@NotNull BuyCycle buyCycle,@Pattern(regexp="(MON|TUE|WED|THU|FRI)(,(MON|TUE|WED|THU|FRI))*") String buyDayCode,@Min(1) @Max(31) Integer buyDayNumber,@Pattern(regexp="([1-9]|[12][0-9]|3[01])(,([1-9]|[12][0-9]|3[01]))*") String buyDayNumbers,@NotNull @Pattern(regexp="DAILY|WEEKLY|MONTHLY") String appliedCycle,@Pattern(regexp="(MON|TUE|WED|THU|FRI)(,(MON|TUE|WED|THU|FRI))*") String appliedWeekDays,@Pattern(regexp="([1-9]|[12][0-9]|3[01])(,([1-9]|[12][0-9]|3[01]))*") String appliedMonthDays,@DecimalMin("0") BigDecimal appliedAmount,@NotNull @Pattern(regexp="AMOUNT|QUANTITY") String buyBasis,@NotNull @DecimalMin("0") BigDecimal minimumBuyAmount,@DecimalMin("0") BigDecimal baseBuyQuantity,@DecimalMin("0") BigDecimal buyQuantity,@NotNull RegularBuyStatus buyStatus,@Size(max=500) String pauseReason,@Pattern(regexp="[YN]") String userPauseYn,@Pattern(regexp="[YN]") String autoCalculateYn){}
 public record RegularBuyRow(String regularBuyKey,Long accountId,AccountType accountType,Long stockId,String stockCode,String stockName,BuyCycle buyCycle,String buyDayCode,Integer buyDayNumber,String buyDayNumbers,String buyBasis,BigDecimal minimumBuyAmount,BigDecimal baseBuyQuantity,BigDecimal buyQuantity,BigDecimal recommendedBuyAmount,RegularBuyStatus buyStatus,String pauseReason,String userPauseYn,String autoCalculateYn){}
 ...
 @GetMapping("/holdings") public ApiResponse<List<HoldingRow>> holdings(HttpServletRequest r){List<TbHold> rows=holdings.findAll().stream().filter(x->"N".equals(x.getDeleteYn())).toList();Map<Long,BigDecimal> totals=rows.stream().filter(x->"Y".equals(x.getUseYn())).collect(Collectors.groupingBy(x->x.getAccount().getAccountId(),Collectors.reducing(BigDecimal.ZERO,x->nvl(x.getEvaluationAmount()),BigDecimal::add)));Map<String,TbRegBuy> buySettings=regularBuys.findAll().stream().filter(x->"N".equals(x.getDeleteYn())).collect(Collectors.toMap(x->holdingKey(x.getAccount().getAccountId(),x.getStock().getStockId()),x->x,(a,b)->a));return ok(rows.stream().map(x->row(x,totals.getOrDefault(x.getAccount().getAccountId(),BigDecimal.ZERO),buySettings.get(holdingKey(x.getAccount().getAccountId(),x.getStock().getStockId())))).toList(),r);}
 ...
 @GetMapping("/investment-grades") public ApiResponse<List<Map<String,Object>>> investmentGrades(HttpServletRequest r){return ok(jdbc.sql("""
 ...
 private void apply(TbRegBuy x,RegularBuyRequest b){TbAcct a=account(b.accountId());TbStk s=stock(b.stockId());AccountType accountType=a.getAccountType();x.setAccountType(accountType);x.setStockCode(s.getStockCode());x.setLegacyStockName(s.getStockName());x.setAccount(a);x.setStock(s);x.setPriority(b.priority());x.setInvestmentGrade(investmentGrade(b.investmentGrade()));x.setMemo(b.memo());x.setBuyCycle(BuyCycle.WEEKLY);x.setBuyDayCode("TUE");x.setBuyDayNumber(null);x.setBuyDayNumbers(null);applyAppliedSchedule(x,b);x.setAppliedAmount(b.appliedAmount());x.setBuyBasis(b.buyBasis());x.setMinimumBuyAmount(new BigDecimal("10000"));x.setBaseBuyQuantity(BigDecimal.ONE);x.setBuyQuantity(b.buyQuantity());RegularBuyStatus status=b.buyStatus();String userPause=status==RegularBuyStatus.ACTIVE?nvl(b.userPauseYn(),"N"):"N";x.setBuyStatus(status);x.setPauseReason(b.pauseReason());x.setUserPauseYn(userPause);x.setAutoCalculateYn(nvl(b.autoCalculateYn(),"Y"));x.setLegacyActiveYn(status==RegularBuyStatus.ACTIVE?"Y":"N");x.setDeleteYn("N");}
 private String investmentGrade(String value){if(value==null||value.isBlank())return null;String grade=value.trim();Long count=jdbc.sql("SELECT COUNT(*) FROM \"TB_CD_DTL\" WHERE \"CD_GRP\"='INVESTMENT_GRADE' AND \"CD_NM\"=:grade AND \"ACTV_YN\"='Y'").param("grade",grade).query(Long.class).single();if(count==0)throw new BusinessException(ErrorCode.INVALID_REQUEST);return grade;}
 private void createDefaultRegularBuy(TbHold holding){AccountType accountType=holding.getAccount().getAccountType();boolean quantityDefault=accountType==AccountType.ISA||accountType==AccountType.PENSION;TbRegBuy x=new TbRegBuy();x.setAccountType(accountType);x.setStockCode(holding.getStock().getStockCode());x.setLegacyStockName(holding.getStock().getStockName());x.setLegacyCycleType("MONTHLY");x.setLegacyMonthDay(15);x.setAppliedMonthDays("15");x.setLegacyActiveYn("N");x.setAccount(holding.getAccount());x.setStock(holding.getStock());x.setBuyCycle(BuyCycle.WEEKLY);x.setBuyDayCode("TUE");x.setBuyDayNumbers(null);x.setBuyBasis(quantityDefault?"QUANTITY":"AMOUNT");x.setMinimumBuyAmount(new BigDecimal("10000"));x.setBaseBuyQuantity(BigDecimal.ONE);x.setBuyStatus(RegularBuyStatus.PAUSED);x.setPauseReason("기본 설정");x.setUserPauseYn("N");x.setAutoCalculateYn("Y");x.setRuleVersionNumber(1);x.setDeleteYn("N");regularBuys.save(x);}
 private void applyAppliedSchedule(TbRegBuy x,RegularBuyRequest b){x.setLegacyCycleType(b.appliedCycle());x.setLegacyWeekDays(b.appliedCycle().equals("WEEKLY")?b.appliedWeekDays():null);String monthDays=b.appliedCycle().equals("MONTHLY")?b.appliedMonthDays():null;x.setAppliedMonthDays(monthDays);x.setLegacyMonthDay(monthDays==null?null:Integer.valueOf(monthDays.split(",")[0]));}
 ...
 private HoldingRow row(TbHold x){TbRegBuy buy=regularBuys.findByAccount_AccountIdAndStock_StockIdAndDeleteYn(x.getAccount().getAccountId(),x.getStock().getStockId(),"N").orElse(null);return row(x,accountEvaluationTotal(x.getAccount().getAccountId()),buy);}
 private HoldingRow row(TbHold x,BigDecimal accountTotal,TbRegBuy buy){BigDecimal currentWeight=x.getCurrentWeight();if(currentWeight==null&&accountTotal.signum()>0)currentWeight=nvl(x.getEvaluationAmount()).multiply(new BigDecimal("100")).divide(accountTotal,4,RoundingMode.HALF_UP);return new HoldingRow(x.getHoldingId(),x.getAccount().getAccountId(),x.getAccount().getAccountType(),x.getStock().getStockId(),x.getStock().getStockCode(),x.getStock().getStockName(),x.getHoldingQuantity(),x.getAveragePrice(),x.getCurrentPrice(),x.getExchangeRate(),x.getEvaluationAmount(),x.getProfitLossRate(),x.getTargetWeight(),currentWeight,x.getHoldingStatus(),buy==null?null:buy.getBuyStatus(),buy==null?"N":buy.getUserPauseYn(),x.getMemo(),x.getUseYn());}
 private String holdingKey(Long accountId,Long stockId){return accountId+":"+stockId;}
 private BigDecimal accountEvaluationTotal(Long accountId){return jdbc.sql("SELECT COALESCE(SUM(COALESCE(\"EVL_AMT\",0)),0) FROM \"TB_HOLD\" WHERE \"ACCT_ID\"=:accountId AND \"DEL_YN\"='N' AND \"USE_YN\"='Y'").param("accountId",accountId).query(BigDecimal.class).single();}
 private RegularBuyRow row(TbRegBuy x){return new RegularBuyRow(x.getAccountType()+"/"+x.getStockCode(),x.getAccount().getAccountId(),x.getAccount().getAccountType(),x.getStock().getStockId(),x.getStock().getStockCode(),x.getStock().getStockName(),x.getBuyCycle(),x.getBuyDayCode(),x.getBuyDayNumber(),x.getBuyDayNumbers(),x.getBuyBasis(),x.getMinimumBuyAmount(),x.getBaseBuyQuantity(),x.getBuyQuantity(),x.getRecommendedBuyAmount(),x.getBuyStatus(),x.getPauseReason(),x.getUserPauseYn(),x.getAutoCalculateYn());}
 ...
 private List<Map<String,Object>> regularBuyRows(){return jdbc.sql("""
  ...
  """).query().listOfRows();} private BigDecimal nvl(BigDecimal v){return v==null?BigDecimal.ZERO:v;} private String nvl(String v,String d){return v==null?d:v;} private <T> ApiResponse<T> ok(T d,HttpServletRequest r){return ApiResponse.success(d,TraceIdUtils.resolve(r));}
}
