...
public class OperationalDataAdminController {
 ...
 public record HoldingRow(Long holdingId,Long accountId,AccountType accountType,Long stockId,String stockCode,String stockName,BigDecimal holdingQuantity,BigDecimal averagePrice,BigDecimal currentPrice,BigDecimal exchangeRate,BigDecimal evaluationAmount,BigDecimal profitLossRate,BigDecimal targetWeight,BigDecimal currentWeight,HoldingStatus holdingStatus,String memo,String useYn){}
 ...
 @GetMapping("/holdings") public ApiResponse<List<HoldingRow>> holdings(HttpServletRequest r){List<TbHold> rows=holdings.findAll().stream().filter(x->"N".equals(x.getDeleteYn())).toList();Map<Long,BigDecimal> totals=rows.stream().filter(x->"Y".equals(x.getUseYn())).collect(Collectors.groupingBy(x->x.getAccount().getAccountId(),Collectors.reducing(BigDecimal.ZERO,x->nvl(x.getEvaluationAmount()),BigDecimal::add)));return ok(rows.stream().map(x->row(x,totals.getOrDefault(x.getAccount().getAccountId(),BigDecimal.ZERO))).toList(),r);}
 ...
 private HoldingRow row(TbHold x){return row(x,accountEvaluationTotal(x.getAccount().getAccountId()));}
 private HoldingRow row(TbHold x,BigDecimal accountTotal){BigDecimal currentWeight=x.getCurrentWeight();if(currentWeight==null&&accountTotal.signum()>0)currentWeight=nvl(x.getEvaluationAmount()).multiply(new BigDecimal("100")).divide(accountTotal,4,RoundingMode.HALF_UP);return new HoldingRow(x.getHoldingId(),x.getAccount().getAccountId(),x.getAccount().getAccountType(),x.getStock().getStockId(),x.getStock().getStockCode(),x.getStock().getStockName(),x.getHoldingQuantity(),x.getAveragePrice(),x.getCurrentPrice(),x.getExchangeRate(),x.getEvaluationAmount(),x.getProfitLossRate(),x.getTargetWeight(),currentWeight,x.getHoldingStatus(),x.getMemo(),x.getUseYn());}
