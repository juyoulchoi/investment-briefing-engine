...
public class OperationalDataAdminController {
 ...
 private void apply(TbRegBuy x,RegularBuyRequest b){TbAcct a=account(b.accountId());TbStk s=stock(b.stockId());x.setAccountType(a.getAccountType());x.setStockCode(s.getStockCode());x.setLegacyStockName(s.getStockName());x.setAccount(a);x.setStock(s);x.setPriority(b.priority());x.setInvestmentGrade(investmentGrade(b.investmentGrade()));x.setMemo(b.memo());x.setBuyCycle(b.buyCycle());x.setBuyDayCode(b.buyCycle()==BuyCycle.WEEKLY?b.buyDayCode():null);x.setBuyDayNumber(null);x.setBuyDayNumbers(b.buyCycle()==BuyCycle.MONTHLY?b.buyDayNumbers():null);applyAppliedSchedule(x,b);x.setAppliedAmount(b.appliedAmount());x.setBuyBasis(b.buyBasis());x.setMinimumBuyAmount(b.minimumBuyAmount());x.setMaximumBuyAmount(b.maximumBuyAmount());x.setBaseBuyQuantity(b.baseBuyQuantity());x.setBuyQuantity(b.buyQuantity());x.setMaximumMultiplier(b.maximumMultiplier());RegularBuyStatus status=b.buyStatus();String userPause=status==RegularBuyStatus.ACTIVE?nvl(b.userPauseYn(),"N"):"N";x.setBuyStatus(status);x.setPauseReason(b.pauseReason());x.setUserPauseYn(userPause);x.setAutoCalculateYn(nvl(b.autoCalculateYn(),"Y"));x.setLegacyActiveYn(status==RegularBuyStatus.ACTIVE?"Y":"N");x.setDeleteYn("N");}
 ...
 private void createDefaultRegularBuy(TbHold holding){TbRegBuy x=new TbRegBuy();x.setAccountType(holding.getAccount().getAccountType());x.setStockCode(holding.getStock().getStockCode());x.setLegacyStockName(holding.getStock().getStockName());x.setLegacyCycleType("MONTHLY");x.setLegacyMonthDay(15);x.setAppliedMonthDays("15");x.setLegacyActiveYn("N");x.setAccount(holding.getAccount());x.setStock(holding.getStock());x.setBuyCycle(BuyCycle.MONTHLY);x.setBuyDayNumbers("15");x.setBuyBasis("AMOUNT");x.setMinimumBuyAmount(BigDecimal.ZERO);x.setMaximumMultiplier(new BigDecimal("3"));x.setBuyStatus(RegularBuyStatus.PAUSED);x.setPauseReason("기본 설정");x.setUserPauseYn("N");x.setAutoCalculateYn("Y");x.setRuleVersionNumber(1);x.setDeleteYn("N");regularBuys.save(x);}
