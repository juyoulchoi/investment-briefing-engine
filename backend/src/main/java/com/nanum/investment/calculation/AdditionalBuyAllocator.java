...
@Component
public class AdditionalBuyAllocator {
  public List<Allocation> allocate(BigDecimal budget, List<Candidate> candidates) {
    if (budget == null || budget.signum() < 0)
      throw new IllegalArgumentException("추가매수 예산은 0 이상이어야 합니다.");
    ...
    for (Candidate c : sorted) {
      BigDecimal amount =
          min(nonNegative(c.maximumAmount()), nonNegative(c.weightGapAmount()), remaining);
  ...
  private BigDecimal min(BigDecimal... v) {
