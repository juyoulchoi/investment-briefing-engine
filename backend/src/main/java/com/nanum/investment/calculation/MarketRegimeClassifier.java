...
public class MarketRegimeClassifier {
    public CalculationResult<MarketRegime> classify(MarketRegimeInput i, MarketRuleSet rules) {
        if (i == null || i.marketScore() == null)
            throw new IllegalArgumentException("시장점수는 필수입니다.");
        if (i.emergencyEvent())
        ...
        MarketRegime regime = s.compareTo(new BigDecimal("80")) >= 0 ? MarketRegime.OVERHEATED
                : s.compareTo(new BigDecimal("60")) >= 0 ? MarketRegime.NORMAL
                        : s.compareTo(new BigDecimal("45")) >= 0 ? MarketRegime.MILD_CORRECTION
                                : s.compareTo(new BigDecimal("25")) >= 0 ? MarketRegime.STRONG_CORRECTION
