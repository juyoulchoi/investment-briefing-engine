...
public class ExchangeRateService {
  ...
  private void recalculate() {
    jdbc.sql(
            """
  WITH x AS (SELECT "BASE_DT","BASE_CURR_CD","QUOTE_CURR_CD",
   lag("EXCH_RT") OVER(PARTITION BY "BASE_CURR_CD","QUOTE_CURR_CD" ORDER BY "BASE_DT") prev,
   max("EXCH_RT") OVER(PARTITION BY "BASE_CURR_CD","QUOTE_CURR_CD" ORDER BY "BASE_DT" ROWS BETWEEN 251 PRECEDING AND CURRENT ROW) high52,
   min("EXCH_RT") OVER(PARTITION BY "BASE_CURR_CD","QUOTE_CURR_CD" ORDER BY "BASE_DT" ROWS BETWEEN 251 PRECEDING AND CURRENT ROW) low52
   FROM "TB_EXCH_DAY" WHERE "BASE_CURR_CD"=:base AND "QUOTE_CURR_CD"=:quote)
  UPDATE "TB_EXCH_DAY" d SET "PREV_EXCH_RT"=x.prev,
   ...
   "HIGH_52W_RT"=x.high52,"LOW_52W_RT"=x.low52 FROM x
   WHERE d."BASE_DT"=x."BASE_DT"
     AND d."BASE_CURR_CD"=x."BASE_CURR_CD"
     AND d."QUOTE_CURR_CD"=x."QUOTE_CURR_CD"
  """)
        .param("base", BASE)
        .param("quote", QUOTE)
        .update();
