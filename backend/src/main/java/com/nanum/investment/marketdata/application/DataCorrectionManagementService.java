...
public class DataCorrectionManagementService {
  ...
  public List<Map<String, Object>> recalculations(String requestedStatus, int limit) {
    ...
    return jdbc.sql(
            "SELECT \"RECALC_ID\" recalculation_id,\"CORR_ID\" correction_id,\"TARGET_TP\" target_type,\"BASE_DT\" base_date,\"STATUS\" status,\"ATTEMPT_CNT\" attempt_count,\"ERROR_MSG\" error_message,\"CRT_DTTM\" created_at,\"START_DTTM\" started_at,\"COMPLETE_DTTM\" completed_at FROM \"TB_RECALC_QUEUE\" WHERE \"STATUS\"=:status ORDER BY \"RECALC_ID\" LIMIT :limit")
        .param("status", status)
        .param("limit", Math.min(Math.max(limit, 1), 1000))
        .query()
        .listOfRows();
  ...
  public void transition(long id, String requestedStatus, String error) {
    ...
    int updated =
        jdbc.sql(
                "UPDATE \"TB_RECALC_QUEUE\" SET \"STATUS\"=:status,\"ATTEMPT_CNT\"=CASE WHEN :status='RUNNING' THEN \"ATTEMPT_CNT\"+1 ELSE \"ATTEMPT_CNT\" END,\"START_DTTM\"=CASE WHEN :status='RUNNING' THEN CURRENT_TIMESTAMP ELSE \"START_DTTM\" END,\"COMPLETE_DTTM\"=CASE WHEN :status IN ('COMPLETED','FAILED','IGNORED') THEN CURRENT_TIMESTAMP ELSE NULL END,\"ERROR_MSG\"=:error WHERE \"RECALC_ID\"=:id")
            .param("status", status)
            .param("error", error)
            .param("id", id)
            .update();
