...
public class MarketSnapshotGenerationService {
  ...
  public MarketSnapshotGenerationService(MarketDataConsistencyService validation, JdbcClient jdbc) {
    this.validation = validation;
    this.jdbc = jdbc;
  }

  @Transactional
  public MarketSnapshotGenerationResult generate(LocalDate baseDate) {
    MarketDataValidationResult checked = validation.validateOrThrow(baseDate);
    Fx fx = latestFx(baseDate);
    SourceSnapshot kr = krSnapshot(baseDate, fx);
    SourceSnapshot us = usSnapshot(baseDate, fx);
    List<MarketSnapshotGenerationResult.Snapshot> saved =
        List.of(save(baseDate, kr, checked.dataStatus()), save(baseDate, us, checked.dataStatus()));
    return new MarketSnapshotGenerationResult(
        baseDate, checked.dataStatus(), checked.confidence(), saved, checked.warnings());
  }

  public List<Map<String, Object>> find(LocalDate baseDate) {
    return jdbc.sql(
            """
  ...
  private SourceSnapshot krSnapshot(LocalDate baseDate, Fx fx) {
   ...
   """)
            ...
            .query(
                (rs, n) ->
                    new Breadth(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getBigDecimal(4)))
            .single();
  ...
  private SourceSnapshot usSnapshot(LocalDate baseDate, Fx fx) {
    IndexPoint index = latestIndex("SP500", baseDate);
        ...
        .query(
            (rs, n) ->
                new IndexPoint(
                    ...
                    rs.getBigDecimal(4)))
        .optional()
        .orElseThrow(() -> new IllegalStateException(code + " 대표지수가 없습니다."));
  }

  private Fx latestFx(LocalDate baseDate) {
    return jdbc.sql(
            "SELECT \"BASE_DT\",\"EXCH_RT\",\"CHG_RT\" FROM \"TB_EXCH_DAY\" WHERE \"BASE_CURR_CD\"='USD' AND \"QUOTE_CURR_CD\"='KRW' AND \"BASE_DT\"<=:day ORDER BY \"BASE_DT\" DESC LIMIT 1")
        .param("day", baseDate)
        .query(
            (rs, n) ->
                new Fx(rs.getObject(1, LocalDate.class), rs.getBigDecimal(2), rs.getBigDecimal(3)))
        .optional()
        .orElseThrow(() -> new IllegalStateException("USD/KRW 환율이 없습니다."));
  }

  private MarketSnapshotGenerationResult.Snapshot save(
      LocalDate baseDate, SourceSnapshot source, DataStatus status) {
    BigDecimal breadthRate = breadthRate(source.breadth());
    int age =
        (int)
            Math.min(
                Integer.MAX_VALUE,
                ChronoUnit.MINUTES.between(
                    source.index().date().atStartOfDay(), baseDate.plusDays(1).atStartOfDay()));
    Long id =
        jdbc.sql(
                """
   ...
   """)
            ...
            .param("indexValue", source.index().value())
            .param("indexChange", source.index().change())
            .param("fx", source.fx().rate())
            .param("fxChange", source.fx().change())
            .param("volatility", source.volatility() == null ? null : source.volatility().value())
            .param(
                "volatilityChange",
                source.volatility() == null ? null : source.volatility().change())
            .param("advancing", source.breadth().advancing())
            .param("declining", source.breadth().declining())
            .param("unchanged", source.breadth().unchanged())
            .param("breadth", breadthRate)
            .param("turnover", source.breadth().turnover())
            .param("source", source.source())
            .param("status", status.name())
            .param("age", age)
            .param("reference", "index=" + source.index().date() + ",fx=" + source.fx().date())
            .query(Long.class)
            .single();
    return new MarketSnapshotGenerationResult.Snapshot(
        id,
        source.code(),
        source.name(),
        source.index().date(),
        ...
        source.index().change(),
        source.fx().rate(),
        source.volatility() == null ? null : source.volatility().value(),
        source.breadth().advancing(),
        source.breadth().declining(),
        source.breadth().unchanged(),
        breadthRate);
  }

  private BigDecimal breadthRate(Breadth value) {
    int directional = value.advancing() + value.declining();
    return directional == 0
        ? null
        : BigDecimal.valueOf(value.advancing())
