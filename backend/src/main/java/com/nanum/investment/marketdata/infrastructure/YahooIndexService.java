...
public class YahooIndexService {
  ...
  public CollectionResult refresh(String indexCode) {
    return collect(indexCode, LocalDate.now().minusDays(10), LocalDate.now());
  }
  ...
  public List<CollectionResult> collectAll(LocalDate from, LocalDate to) {
    return yahooIndices().stream().map(i -> collect(i.code(), from, to)).toList();
  }
  ...
  public Map<String, Object> latest(String requestedCode) {
    IndexInfo index = findIndex(requestedCode);
    var rows = history(index.code(), LocalDate.now().minusYears(1), LocalDate.now());
    if (rows.isEmpty()) throw new IllegalArgumentException("저장된 지수 시세가 없습니다.");
    return rows.getLast();
  }
  ...
  private int save(IndexInfo index, JsonNode result, LocalDate from, LocalDate to) {
    ...
    for (int i = 0; i < timestamps.size(); i++) {
      ...
      BigDecimal change = previous == null ? null : close.subtract(previous),
          rate =
    ...
    """)
          ...
          .param("change", change)
          .param("rate", rate)
          .param("open", decimal(value(quote.path("open"), i)))
  ...
  private String normalize(String code) {
    ...
    if (!value.matches("[A-Z0-9_]{1,30}")) throw new IllegalArgumentException("유효하지 않은 지수 코드입니다.");
    return value;
  }

  private JsonNode value(JsonNode array, int i) {
    return array == null || !array.isArray() || i >= array.size() ? null : array.get(i);
  }
