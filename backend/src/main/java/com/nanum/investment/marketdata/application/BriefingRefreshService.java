...
import com.nanum.investment.regularbuy.application.RegularBuyRecommendationSyncService;
import java.time.*;
import java.util.*;
...
public class BriefingRefreshService {
  private final HoldingMarketDataRefreshService holdings;
  private final ExchangeRateService exchange;
  private final FredBondYieldService bonds;
  ...
  public BriefingRefreshService(
      HoldingMarketDataRefreshService holdings,
      ExchangeRateService exchange,
      FredBondYieldService bonds,
      MarketDataConsistencyService validation,
      MarketSnapshotGenerationService snapshots,
      MarketSentimentGenerationService sentiments,
  ...
  private BriefingRefreshResult refresh(boolean publish, boolean syncRecommendations) {
    ...
    List<String> completed = new ArrayList<>(), failures = new ArrayList<>();
    Map<String, Object> results = new LinkedHashMap<>();
    HoldingMarketDataRefreshResult market = holdings.refresh();
    results.put("MARKET_DATA", market);
    if (market.failures().isEmpty()) completed.add("1-2 국내·해외·ETF·지수 수집");
    else failures.addAll(market.failures());
    attempt(
        "3 환율 수집", completed, failures, results, () -> exchange.collect(day.minusDays(10), day));
    attempt("4 채권금리 수집", completed, failures, results, () -> bonds.collect(day.minusDays(10), day));
    boolean quality =
        attempt("5 정합성 검증", completed, failures, results, () -> validation.validateOrThrow(day));
    boolean snapshot =
        quality
            && attempt("6 시장 스냅샷 생성", completed, failures, results, () -> snapshots.generate(day));
    boolean sentiment =
        snapshot
            && attempt("7 시장심리 계산", completed, failures, results, () -> sentiments.generate(day));
    boolean decision =
        sentiment
            && attempt("8 투자판단 생성", completed, failures, results, () -> decisions.generate(day));
    boolean recommendation =
        decision
            && (!syncRecommendations
                || attempt(
                    "8-1 추천금액 반영", completed, failures, results, () -> recommendations.sync(day)));
    boolean plan =
        recommendation
            && attempt(
    ...
    return new BriefingRefreshResult(
        ...
        day,
        market.krxBaseDate(),
        market.krxReceivedCounts(),
        market.overseasRequestedCount(),
        market.overseasSuccessCount(),
        market.overseasSuccessSymbols(),
  ...
  private boolean attempt(
      ...
      Step step) {
    try {
      results.put(name, step.run());
      completed.add(name);
      return true;
    } catch (Exception e) {
      failures.add(name + ": " + rootMessage(e));
      return false;
    }
  }
