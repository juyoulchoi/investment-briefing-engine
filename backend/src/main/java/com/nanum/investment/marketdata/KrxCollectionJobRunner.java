        ...
        try {
            for (KrxDataset dataset : KrxDataset.values()) {
                ...
                try {
                    ...
                    jobs.saveItem(jobId, dataset.name(), "SUCCESS", result.receivedCount(), result.storedCount(), null,
                            startedAt);
                } catch (RestClientResponseException exception) {
                    jobs.saveItem(jobId, dataset.name(), "HTTP_" + exception.getStatusCode().value(), 0, 0,
                            trim(exception.getResponseBodyAsString()), startedAt);
                } catch (RuntimeException exception) {
                    jobs.saveItem(jobId, dataset.name(), "FAILED", 0, 0, trim(exception.getMessage()), startedAt);
                }
            }
        } finally {
