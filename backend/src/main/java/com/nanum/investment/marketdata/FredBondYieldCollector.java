...
public class FredBondYieldCollector implements BondYieldCollector {
 ...
 public FredBondYieldCollector(@Value("${fred.base-url}") String baseUrl,@Value("${fred.api-key:}") String apiKey){this.client=RestClient.builder().baseUrl(baseUrl).defaultHeader("Accept","application/json").build();this.apiKey=apiKey;}
 ...
 public List<Yield> collectRange(String requestedCode,LocalDate from,LocalDate to){
  ...
  JsonNode body=client.get().uri(u->u.path("/series/observations").queryParam("series_id",code).queryParam("api_key",apiKey).queryParam("file_type","json").queryParam("observation_start",from).queryParam("observation_end",to).queryParam("sort_order","asc").build()).retrieve().body(JsonNode.class);
