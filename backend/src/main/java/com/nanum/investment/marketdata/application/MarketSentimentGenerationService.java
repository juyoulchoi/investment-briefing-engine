...
import com.nanum.investment.briefing.domain.calculation.MarketSentimentCalculator;
import com.nanum.investment.briefing.domain.calculation.MarketSentimentInput;
...
public class MarketSentimentGenerationService {
  ...
  public MarketSentimentGenerationService(JdbcClient jdbc, MarketSentimentCalculator calculator) {
    this.jdbc = jdbc;
    this.calculator = calculator;
  }

  @Transactional
  public MarketSentimentGenerationResult generate(LocalDate baseDate) {
    if (baseDate == null) throw new IllegalArgumentException("시장심리 기준일이 필요합니다.");
    List<SnapshotInput> inputs = inputs(baseDate);
    if (inputs.size() < 2) throw new IllegalStateException("KR_MARKET·US_MARKET 스냅샷이 모두 필요합니다.");
    return new MarketSentimentGenerationResult(
        baseDate, inputs.stream().map(this::calculateAndSave).toList());
  }

  private List<SnapshotInput> inputs(LocalDate day) {
    return jdbc.sql(
            """
  SELECT s."BASE_DT",s."MKT_SNAP_CD",s."MKT_BREADTH_RT",s."LIQD_SCR",s."FRGN_NET_AMT",s."INST_NET_AMT",
   ...
   COALESCE((SELECT b."CHG_BP" FROM "TB_FRED_BOND_DAY" b WHERE b."BOND_CD"='DGS10' AND b."BASE_DT"<=s."BASE_DT" ORDER BY b."BASE_DT" DESC LIMIT 1),0)
        ...
        .query(
            (rs, n) ->
                new SnapshotInput(
                    ...
                    rs.getBigDecimal(9),
                    rs.getBigDecimal(10),
                    DataStatus.valueOf(rs.getString(11)),
                    rs.getBigDecimal(12)))
        .list();
  }

  private MarketSentimentGenerationResult.Sentiment calculateAndSave(SnapshotInput s) {
    BigDecimal newsFear = negativePressure(s.indexChange(), new BigDecimal("5")),
        aiFatigue = FIFTY,
        earnings = FIFTY;
    BigDecimal liquidity = score(s.liquidity() == null ? s.breadth() : s.liquidity()),
        flow = flowScore(s.foreignNet(), s.institutionNet());
    BigDecimal exchange = positivePressure(s.exchangeChange(), new BigDecimal("3")),
        rate = positivePressure(s.rateChangeBp(), new BigDecimal("30")),
        volatility = volatilityPressure(s.volatility(), s.volatilityChange());
    BigDecimal confidence =
        s.status() == DataStatus.FRESH ? new BigDecimal("85") : new BigDecimal("70");
    if (s.liquidity() == null) confidence = confidence.subtract(new BigDecimal("5"));
    if (s.foreignNet() == null && s.institutionNet() == null)
      confidence = confidence.subtract(new BigDecimal("10"));
    var result =
        calculator.calculate(
            new MarketSentimentInput(
                ...
                confidence));
    boolean structural =
        result.score().compareTo(new BigDecimal("20")) < 0
            && (volatility.compareTo(new BigDecimal("80")) >= 0
                || s.indexChange() != null && s.indexChange().compareTo(new BigDecimal("-5")) <= 0);
    String reason =
        "확산도 "
            + one(liquidity)
            + ", 수급 "
   ...
   """)
            ...
            .param("phase", result.phase().name())
            .param("confidence", result.confidenceRate())
            .param("structural", structural ? "Y" : "N")
            .param("damage", structural ? new BigDecimal("80") : new BigDecimal("20"))
            .param("reason", reason)
            .param("status", s.status().name())
            .query(Long.class)
            .single();
    return new MarketSentimentGenerationResult.Sentiment(
        id,
        s.code(),
        result.score(),
        result.phase(),
  ...
  private BigDecimal flowScore(BigDecimal foreignNet, BigDecimal institutionNet) {
    if (foreignNet == null && institutionNet == null) return FIFTY;
    return clamp(
        FIFTY.add(
            value(foreignNet)
                .add(value(institutionNet))
                .divide(new BigDecimal("100000000000"), 4, RoundingMode.HALF_UP)));
  }

  private BigDecimal volatilityPressure(BigDecimal level, BigDecimal change) {
    if (level == null) return positivePressure(change, new BigDecimal("10"));
    return clamp(
        level
            .subtract(BigDecimal.TEN)
            .multiply(new BigDecimal("3"))
            .add(value(change).max(BigDecimal.ZERO)));
  }

  private BigDecimal negativePressure(BigDecimal change, BigDecimal scale) {
    return clamp(
        value(change)
            .negate()
            .max(BigDecimal.ZERO)
            .multiply(HUNDRED)
            .divide(scale, 4, RoundingMode.HALF_UP));
  }

  private BigDecimal positivePressure(BigDecimal change, BigDecimal scale) {
    return clamp(
        value(change)
            .max(BigDecimal.ZERO)
            .multiply(HUNDRED)
  ...
  private BigDecimal score(BigDecimal value) {
    return value == null ? FIFTY : clamp(value);
  }
  ...
  private BigDecimal clamp(BigDecimal value) {
    return value.max(BigDecimal.ZERO).min(HUNDRED).setScale(4, RoundingMode.HALF_UP);
  }
