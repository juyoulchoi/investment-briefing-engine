...
public class HoldingMarketDataRefreshService {
 ...
 private LocalDate refreshKrx(Map<String,Integer> counts,List<String> failures){
  LocalDate candidate=latestCompletedKrxDate();String lastError=null;
  for(int attempt=0;attempt<8;attempt++,candidate=candidate.minusDays(1)){
   if(candidate.getDayOfWeek()==DayOfWeek.SATURDAY||candidate.getDayOfWeek()==DayOfWeek.SUNDAY)continue;
   Map<String,Integer> current=new LinkedHashMap<>();boolean complete=true;
   for(KrxDataset dataset:KRX_DATASETS)try{var result=krx.collect(dataset,candidate);current.put(dataset.name(),result.receivedCount());if(result.receivedCount()==0)complete=false;}catch(Exception e){lastError=e.getMessage();complete=false;break;}
   if(complete){counts.putAll(current);return candidate;}
   if(lastError!=null&&lastError.contains("KRX_AUTH_KEY"))break;
  }
  failures.add("국내·ETF KRX 수집 실패"+(lastError==null?" 또는 최근 거래일 데이터 없음":": "+lastError));return null;
 }

 private LocalDate latestCompletedKrxDate(){ZonedDateTime now=ZonedDateTime.now(ZoneId.of("Asia/Seoul"));LocalDate date=now.toLocalDate();if(now.getHour()<16)date=date.minusDays(1);while(date.getDayOfWeek()==DayOfWeek.SATURDAY||date.getDayOfWeek()==DayOfWeek.SUNDAY)date=date.minusDays(1);return date;}
 private List<String> overseasSymbols(){return jdbc.sql("""
