...
public class KrxCollectionJobRepository {
    ...
    public void saveItem(UUID jobId, String dataset, String status, int receivedCount,
        ...
        jdbc.update("""
                ...
                """, new MapSqlParameterSource().addValue("jobId", jobId).addValue("dataset", dataset)
                .addValue("status", status).addValue("receivedCount", receivedCount)
                .addValue("storedCount", storedCount).addValue("errorMessage", errorMessage)
    ...
    public JobView find(UUID id) {
        ...
        return new JobView(job.jobId(), job.baseDate(), job.status(), job.totalCount(), job.successCount(),
                job.failedCount(), job.createdAt(), job.startedAt(), job.completedAt(), items);
    }
    ...
    public record JobView(UUID jobId, LocalDate baseDate, String status, int totalCount, int successCount,
            int failedCount, LocalDateTime createdAt, LocalDateTime startedAt, LocalDateTime completedAt,
            List<ItemView> items) {
    }

    public record ItemView(String dataset, String status, int receivedCount, long storedCount,
