...
public class BriefingRefreshService {
 ...
 private final AutomaticInvestmentDecisionService decisions;private final RegularBuyRecommendationSyncService recommendations;private final DailyBuyPlanService plans;private final AutomaticRebalanceService rebalancing;private final BriefingRawDataService rawData;private final InvestmentBriefingService briefing;
 public BriefingRefreshService(HoldingMarketDataRefreshService holdings,ExchangeRateService exchange,FredBondYieldService bonds,
  ...
  AutomaticInvestmentDecisionService decisions,RegularBuyRecommendationSyncService recommendations,DailyBuyPlanService plans,AutomaticRebalanceService rebalancing,BriefingRawDataService rawData,InvestmentBriefingService briefing){this.holdings=holdings;this.exchange=exchange;this.bonds=bonds;this.validation=validation;this.snapshots=snapshots;this.sentiments=sentiments;this.decisions=decisions;this.recommendations=recommendations;this.plans=plans;this.rebalancing=rebalancing;this.rawData=rawData;this.briefing=briefing;}

 public BriefingRefreshResult refresh(){return refresh(true,false);}
 public BriefingRefreshResult refreshWeeklyDecision(){return refresh(false,true);}

 private BriefingRefreshResult refresh(boolean publish,boolean syncRecommendations){
  ...
  HoldingMarketDataRefreshResult market=holdings.refresh();results.put("MARKET_DATA",market);if(market.failures().isEmpty())completed.add("1-2 국내·해외·ETF·지수 수집");else failures.addAll(market.failures());
  ...
  boolean recommendation=decision&&(!syncRecommendations||attempt("8-1 추천금액 반영",completed,failures,results,()->recommendations.sync(day)));
  boolean plan=recommendation&&attempt("9 추가매수·재매수 계산",completed,failures,results,()->plans.calculateAndSave(day));
  boolean rebalance=plan&&attempt("9 리밸런싱 계산",completed,failures,results,()->rebalancing.generate(day));
  boolean raw=rebalance&&attempt("10 브리핑 원천데이터 생성",completed,failures,results,()->rawData.generate(day));
  if(raw&&publish)attempt("11-13 OpenAI 설명·검증·발행",completed,failures,results,briefing::generateAndSave);
