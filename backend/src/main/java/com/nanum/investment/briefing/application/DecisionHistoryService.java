...
public class DecisionHistoryService {
  ...
  public void save(InvestmentDecisionRequest request, PortfolioDecision decision) {
    jdbc.sql(
            "UPDATE \"TB_INV_DEC\" SET \"LATEST_YN\"='N' WHERE \"BASE_DT\"=:date AND \"ACCT_ID\" IS NULL AND \"MKT_SNAP_CD\"='GLOBAL' AND \"LATEST_YN\"='Y'")
    ...
    for (StockDecision stock : decision.stockDecisions()) {
      int inserted =
          jdbc.sql(
              ...
              .param("decisionId", id)
              .param("date", decision.decisionDate())
              .param("account", stock.account())
              .param("code", stock.code())
              .param("name", stock.name())
              .param("action", stock.action().name())
              .param("multiplier", stock.multiplier())
              .param("minimum", stock.minimumBuyAmount())
              .param("maximum", stock.maximumBuyAmount())
              .param("maximumMultiplier", stock.maximumIncreaseMultiplier())
              .param("recommended", stock.recommendedBuyAmount())
              .param("reserved", stock.reservedCash())
              .param("adjustmentReason", stock.adjustmentReason())
              .param("cashPlan", stock.cashPlan())
              .param("reasons", toJson(stock.reasons()))
              .update();
      if (inserted != 1)
        throw new IllegalStateException(
            "종목판단 저장 대상 계좌·종목을 정확히 찾을 수 없습니다: " + stock.account() + "/" + stock.code());
    }
  }
  ...
  private String toJson(Object value) {
    try {
      ...
      throw new IllegalStateException("의사결정 이력 JSON 변환에 실패했습니다.", e);
    }
  }
}
