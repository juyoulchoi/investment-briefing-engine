  ...
  private RawBriefing latestRawBriefing(BriefingType briefingType) {
    return jdbc.sql(
            """
                ...
                """)
        .param("briefingType", briefingType.name())
        .query(
            (rs, n) ->
                new RawBriefing(rs.getLong(1), rs.getObject(2, LocalDate.class), rs.getString(3)))
        .optional()
        .orElseThrow(() -> new IllegalStateException(briefingType + " 최신 브리핑 원천데이터가 없습니다."));
  }
