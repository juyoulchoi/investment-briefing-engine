...
public class MarketDirectionPredictionService {
  ...
  public MarketDirectionDto calculateAndSave(LocalDate date) {
    DirectionInput input =
        ...
        """)
            ...
            .optional()
            .orElseThrow(() -> new IllegalStateException("시장방향 예측에 필요한 투자판단이 없습니다."));
    ...
    dates.put("investmentDecision", date);
    dates.put("marketSnapshot", input.snapshotDate());
    dates.put("exchangeRate", input.exchangeDate());
    dates.put("bondYield", input.bondDate());
    ...
    jdbc.sql(
        ...
        .param("seq", sequence)
        .param("score", score)
        .param("up", probabilities[0])
        .param("box", probabilities[1])
        .param("correction", probabilities[2])
        .param("low", probabilities[3])
        .param("upChange", probabilities[0] - previous.up())
        .param("boxChange", probabilities[1] - previous.box())
        .param("correctionChange", probabilities[2] - previous.correction())
        .param("lowChange", probabilities[3] - previous.low())
  ...
  public static void validate(MarketScenarioProbabilityDto value) {
    int[] values = {
      value.uptrendResume(), value.boxRange(), value.reCorrection(), value.retestLow()
    };
