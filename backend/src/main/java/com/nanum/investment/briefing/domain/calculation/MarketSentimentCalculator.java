...
public class MarketSentimentCalculator {
  public Result calculate(MarketSentimentInput i) {
    ...
    BigDecimal score =
        b("100")
            .subtract(n(i.newsFearScore()))
            .multiply(b("0.20"))
            .add(b("100").subtract(n(i.aiFatigueScore())).multiply(b("0.10")))
            .add(n(i.earningsConfidenceScore()).multiply(b("0.20")))
            .add(n(i.liquidityScore()).multiply(b("0.10")))
            .add(n(i.flowScore()).multiply(b("0.15")))
            .add(b("100").subtract(n(i.exchangePressureScore())).multiply(b("0.10")))
    ...
    SentimentPhase phase =
        score.compareTo(b("80")) >= 0
            ? SentimentPhase.GREED
            : score.compareTo(b("65")) >= 0
                ? SentimentPhase.OPTIMISM
                : score.compareTo(b("45")) >= 0
                    ? SentimentPhase.NEUTRAL
                    : score.compareTo(b("30")) >= 0
                        ? SentimentPhase.FATIGUE
                        : score.compareTo(b("15")) >= 0
                            ? SentimentPhase.FEAR
                            : SentimentPhase.PANIC;
    return new Result(
        score,
        phase,
        n(i.dataConfidenceRate()).max(BigDecimal.ZERO).min(b("100")),
