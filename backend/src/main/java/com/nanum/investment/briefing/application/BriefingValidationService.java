...
public class BriefingValidationService {
  ...
  public BriefingValidationService(JdbcClient jdbc, ObjectMapper json) {
    this.jdbc = jdbc;
    this.json = json;
  }

  public void validate(
      Long briefingId, BriefingType expectedType, InvestmentBriefingResponse response) {
    try {
      Source source =
          jdbc.sql(
                  "SELECT \"BASE_DT\",\"BRF_TP\",\"RAW_DATA_JSON\"->'confirmedValues' FROM \"TB_BRF\" WHERE \"BRF_ID\"=:id")
              .param("id", briefingId)
              .query(
                  (rs, n) ->
                      new Source(
              ...
              .optional()
              .orElseThrow(() -> new IllegalStateException("검증할 DB 브리핑 Snapshot이 없습니다."));
      require(
          response != null
      ...
      require(
          expectedType.name().equals(source.type())
              && expectedType.name().equals(response.briefingType()),
          "DAILY/WEEKLY 타입이 다릅니다.");
      require(
          response.items() != null && response.items().size() == BriefingItemCatalog.ITEMS.size(),
          "브리핑은 15개 항목이어야 합니다.");
      List<String> expectedCodes =
          BriefingItemCatalog.ITEMS.stream().map(BriefingItemCatalog.Item::code).toList();
      List<String> actualCodes =
          response.items().stream().map(BriefingItemResponse::itemCode).toList();
      ...
      for (int i = 0; i < response.items().size(); i++) {
        ...
        require(
            BriefingItemCatalog.ITEMS.get(i).title().equals(item.title()),
            item.itemCode() + " 항목 제목이 기준과 다릅니다.");
        require(
            StringUtils.hasText(item.summary())
      ...
      require(
          confirmed != null && source.confirmedValues().equals(confirmed),
          "DB 확정 숫자·등급·신호와 OpenAI 결과가 다릅니다.");
      int risk = confirmed.path("marketRiskScore").asInt(-1),
          direction = confirmed.path("marketDirectionScore").asInt(-1);
      ...
      log.error(
          "브리핑 발행 검증 실패. briefingId={}, type={}, reason={}",
          briefingId,
          expectedType,
          e.getMessage());
      jdbc.sql(
              "UPDATE \"TB_BRF\" SET \"BRF_STS\"='FAILED',\"FAIL_RSN\"=:reason,\"PUBL_YN\"='N',\"UPD_USR_ID\"='VALIDATOR' WHERE \"BRF_ID\"=:id")
          .param("reason", Objects.toString(e.getMessage(), e.getClass().getSimpleName()))
          .param("id", briefingId)
          .update();
      throw e;
    } catch (Exception e) {
  ...
  private JsonNode parse(String value) {
    try {
      ...
      throw new IllegalStateException("DB 확정 JSON을 읽을 수 없습니다.", e);
    }
  }
