...
public class AutomaticInvestmentDecisionService {
  ...
  public AutomaticInvestmentDecisionService(
      JdbcClient jdbc, PortfolioDecisionService decisions, DecisionHistoryService history) {
    this.jdbc = jdbc;
  ...
  @Transactional
  public AutomaticInvestmentDecisionResult generate(LocalDate baseDate) {
    if (baseDate == null) throw new IllegalArgumentException("투자판단 기준일이 필요합니다.");
    ensureSnapshots(baseDate);
    MarketSnapshot market = marketInput(baseDate);
    List<StockPosition> positions = positions(baseDate);
    if (positions.isEmpty()) throw new IllegalStateException("활성 정기매수 종목이 없어 투자판단을 생성할 수 없습니다.");
    long reservedCash = reservedCash();
    InvestmentDecisionRequest request =
        new InvestmentDecisionRequest(baseDate, market, positions, reservedCash);
    PortfolioDecision decision = decisions.decide(request);
    history.save(request, decision);
    return new AutomaticInvestmentDecisionResult(
        baseDate, List.of("KR_MARKET", "US_MARKET"), positions.size(), reservedCash, decision);
  }

  private void ensureSnapshots(LocalDate baseDate) {
    List<String> codes =
        jdbc.sql(
                "SELECT \"MKT_SNAP_CD\" FROM \"TB_MKT_SNAP\" WHERE \"BASE_DT\"=:day AND \"MKT_SNAP_CD\" IN ('KR_MARKET','US_MARKET') AND \"DATA_STS\" IN ('FRESH','PARTIAL')")
            .param("day", baseDate)
            .query(String.class)
            .list();
    if (!codes.containsAll(List.of("KR_MARKET", "US_MARKET")))
      throw new IllegalStateException("검증 완료된 KR_MARKET·US_MARKET 스냅샷이 모두 필요합니다.");
    Long sentiments =
        jdbc.sql(
                "SELECT count(*) FROM \"TB_MKT_SENT\" WHERE \"BASE_DT\"=:day AND \"MKT_SNAP_CD\" IN ('KR_MARKET','US_MARKET','GLOBAL') AND \"DATA_STS\" IN ('FRESH','PARTIAL')")
            .param("day", baseDate)
            .query(Long.class)
            .single();
  ...
  private MarketSnapshot marketInput(LocalDate baseDate) {
   ...
   WITH snapshots AS (SELECT s.*,d."DD_HIGH_RT" index_drawdown FROM "TB_MKT_SNAP" s
    LEFT JOIN LATERAL(SELECT x."DD_HIGH_RT" FROM "TB_IDX_DAY" x WHERE x."IDX_CD"=s."MAIN_IDX_CD" AND x."TRADE_DT"<=s."BASE_DT" ORDER BY x."TRADE_DT" DESC LIMIT 1)d ON true
   ...
   bond AS (SELECT "CHG_BP" FROM "TB_FRED_BOND_DAY" WHERE "BOND_CD"='DGS10' AND "BASE_DT"<=:day ORDER BY "BASE_DT" DESC LIMIT 1)
        ...
        .query(
            (rs, n) ->
                new MarketSnapshot(
                    ...
                    number(rs.getBigDecimal(2)),
                    number(rs.getBigDecimal(3)),
                    number(rs.getBigDecimal(4)),
                    number(rs.getBigDecimal(5)),
                    number(rs.getBigDecimal(6)),
                    number(rs.getBigDecimal(7)),
                    number(rs.getBigDecimal(8)),
                    number(rs.getBigDecimal(9)),
                    score(rs.getBigDecimal(10)),
                    score(rs.getBigDecimal(11)),
                    score(rs.getBigDecimal(12)),
                    score(rs.getBigDecimal(13)),
                    "한국·미국 시장 스냅샷과 확정 시장심리를 종합한 자동 투자판단"))
        .single();
  }

  private List<StockPosition> positions(LocalDate baseDate) {
    return jdbc.sql(
            """
        ...
        .query(
            (rs, n) ->
                new StockPosition(
                    ...
                    number(rs.getBigDecimal(7)),
                    number(rs.getBigDecimal(8)),
                    weight(rs.getBigDecimal(9)),
                    weight(rs.getBigDecimal(10)),
                    number(rs.getBigDecimal(11)),
                    number(rs.getBigDecimal(12)),
                    number(rs.getBigDecimal(13)),
                    rs.getBoolean(14)))
        .list();
  }
  ...
  private int score(BigDecimal value) {
    return (int) Math.max(0, Math.min(100, Math.round(number(value))));
  }

  private long amount(BigDecimal value) {
    return value == null ? 0 : value.setScale(0, java.math.RoundingMode.HALF_UP).longValue();
  }
  ...
  private double weight(BigDecimal value) {
    ...
    return result > 1 ? result / 100.0 : result;
  }
}
