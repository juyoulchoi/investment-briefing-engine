...
public class BriefingSnapshotService {
  ...
  public BriefingSnapshotService(JdbcClient jdbc, ObjectMapper json) {
    this.jdbc = jdbc;
    this.json = json;
  }
    ...
    return jdbc.sql(
        ...
        .query(
            (rs, n) -> {
              ...
              return new FixedBriefingSnapshot(
                  rs.getLong(1),
                  rs.getObject(2, LocalDate.class),
                  BriefingType.valueOf(rs.getString(3)),
                  source);
            })
        .optional()
        .orElseThrow(() -> new IllegalStateException(type + " 최신 확정 브리핑 Snapshot이 없습니다."));
  }
  ...
  private JsonNode parse(String value) {
    try {
      ...
      throw new IllegalStateException("브리핑 Snapshot JSON을 읽을 수 없습니다.", e);
    }
  }

  public record FixedBriefingSnapshot(
      Long briefingId, LocalDate baseDate, BriefingType briefingType, JsonNode source) {}
}
