...
public class BriefingRawDataService {
  ...
  public BriefingRawDataService(JdbcClient jdbc, ObjectMapper json) {
    this.jdbc = jdbc;
    this.json = json;
  }

  @Transactional
  public BriefingRawDataResult generate(LocalDate date) {
    if (date == null) throw new IllegalArgumentException("브리핑 기준일이 필요합니다.");
    DecisionRef decision = decision(date);
    LinkedHashMap<String, Object> sections = sections(date, decision.id());
    DataStatus status = dataStatus(sections);
    int confidence = confidence(decision, sections);
    LinkedHashMap<String, Object> raw = new LinkedHashMap<>();
    raw.put("schemaVersion", "1.0");
    raw.put("baseDate", date);
    raw.put("generatedAt", OffsetDateTime.now(ZoneId.of("Asia/Seoul")));
    ...
    rawJson = toJson(raw);
    jdbc.sql(
            "UPDATE \"TB_BRF\" SET \"LATEST_YN\"='N' WHERE \"BASE_DT\"=:day AND \"BRF_TP\"='DAILY' AND \"SCOPE_TP\"='GLOBAL' AND \"LATEST_YN\"='Y'")
        .param("day", date)
        .update();
    Integer sequence =
        jdbc.sql(
                "SELECT COALESCE(max(\"CALC_SEQ\"),0)+1 FROM \"TB_BRF\" WHERE \"BASE_DT\"=:day AND \"BRF_TP\"='DAILY' AND \"SCOPE_TP\"='GLOBAL'")
            .param("day", date)
            .query(Integer.class)
            .single();
    Long briefingId =
  ...
  public Map<String, Object> find(Long briefingId) {
    List<Map<String, Object>> rows =
        jdbc.sql(
                "SELECT \"BRF_ID\" briefing_id,\"BASE_DT\" base_date,\"CALC_SEQ\" calculation_sequence,\"BRF_STS\" briefing_status,\"DATA_STS\" data_status,\"CONF_RT\" confidence,\"RAW_DATA_JSON\" raw_data FROM \"TB_BRF\" WHERE \"BRF_ID\"=:id")
            .param("id", briefingId)
            .query()
            .listOfRows();
    if (rows.isEmpty()) throw new IllegalArgumentException("브리핑 원천데이터를 찾을 수 없습니다.");
  ...
  private LinkedHashMap<String, Object> sections(LocalDate date, Long decisionId) {
    ...
    out.put(
        "MARKET_SENTIMENT",
        rows(
            "SELECT \"MKT_SNAP_CD\" snapshot_code,\"NEWS_FEAR_SCR\" news_fear_score,\"AI_FATIGUE_SCR\" ai_fatigue_score,\"EARN_CONF_SCR\" earnings_confidence_score,\"LIQD_SCR\" liquidity_score,\"FLOW_SCR\" flow_score,\"EXCH_PRESS_SCR\" exchange_pressure_score,\"RATE_PRESS_SCR\" rate_pressure_score,\"VOL_PRESS_SCR\" volatility_pressure_score,\"SENT_SCR\" sentiment_score,\"SENT_PHASE\" sentiment_phase,\"CONF_RT\" confidence,\"STRUCT_DMG_YN\" structural_damage_yn,\"REV_SIG\" reversal_signal,\"KEY_RSN\" key_reason,\"DATA_STS\" data_status FROM \"TB_MKT_SENT\" WHERE \"BASE_DT\"=:day ORDER BY \"MKT_SNAP_CD\"",
            date,
            null));
    out.put(
        "INVESTMENT_DECISION",
  ...
  private List<Map<String, Object>> rows(String sql, LocalDate date, Long id) {
    var query = jdbc.sql(sql).param("day", date);
    if (id != null) query = query.param("id", id);
    return query.query().listOfRows();
  }

  private Map<String, Object> quality(LocalDate date, Long id) {
    Map<String, Object> result = new LinkedHashMap<>();
    result.put("baseDate", date);
    result.put("snapshotCount", count("TB_MKT_SNAP", "BASE_DT", date));
    result.put("sentimentCount", count("TB_MKT_SENT", "BASE_DT", date));
    result.put(
        "stockDecisionCount",
        jdbc.sql("SELECT count(*) FROM \"TB_STK_DEC\" WHERE \"INV_DEC_ID\"=:id")
            .param("id", id)
            .query(Long.class)
            .single());
    result.put("additionalBuyCount", countByDecision("TB_ADD_BUY", id));
    result.put("rebuyCount", countByDecision("TB_REBUY", id));
    return result;
  }

  private long count(String table, String column, LocalDate date) {
    return jdbc.sql("SELECT count(*) FROM \"" + table + "\" WHERE \"" + column + "\"=:day")
        .param("day", date)
        .query(Long.class)
        .single();
  }

  private long countByDecision(String table, Long id) {
    return jdbc.sql(
            "SELECT count(*) FROM \""
                + table
                + "\" x JOIN \"TB_STK_DEC\" d ON d.\"STK_DEC_ID\"=x.\"STK_DEC_ID\" WHERE d.\"INV_DEC_ID\"=:id")
        .param("id", id)
        .query(Long.class)
        .single();
  }

  private void validateRequired(Map<String, Object> data) {
    for (String key :
        List.of(
            "MARKET_SNAPSHOTS",
            "MARKET_SENTIMENT",
  ...
  private DecisionRef decision(LocalDate date) {
    return jdbc.sql(
            "SELECT \"INV_DEC_ID\",\"CONF_RT\",\"DATA_STS\" FROM \"TB_INV_DEC\" WHERE \"BASE_DT\"=:day AND \"LATEST_YN\"='Y' ORDER BY \"CALC_SEQ\" DESC LIMIT 1")
        .param("day", date)
        .query(
            (rs, n) ->
                new DecisionRef(rs.getLong(1), rs.getInt(2), DataStatus.valueOf(rs.getString(3))))
        .optional()
        .orElseThrow(() -> new IllegalStateException("8단계 투자판단 결과가 없습니다."));
  }

  private DataStatus dataStatus(Map<String, Object> sections) {
    for (Object section : sections.values())
      if (section instanceof List<?> rows)
        for (Object row : rows)
          if (row instanceof Map<?, ?> map) {
            Object status = map.get("data_status");
            if (status != null && !("FRESH".equals(status) || "PARTIAL".equals(status)))
              return DataStatus.ERROR;
            if ("PARTIAL".equals(status)) return DataStatus.PARTIAL;
          }
    return DataStatus.FRESH;
  }

  private int confidence(DecisionRef decision, Map<String, Object> sections) {
    int value = decision.confidence();
    Object sentiment = sections.get("MARKET_SENTIMENT");
    if (sentiment instanceof List<?> rows)
      for (Object row : rows)
        if (row instanceof Map<?, ?> map && map.get("confidence") instanceof Number n)
          value = Math.min(value, n.intValue());
  ...
  private String toJson(Object value) {
    try {
      ...
      throw new IllegalStateException("브리핑 원천데이터 JSON 생성에 실패했습니다.", e);
    }
  }
