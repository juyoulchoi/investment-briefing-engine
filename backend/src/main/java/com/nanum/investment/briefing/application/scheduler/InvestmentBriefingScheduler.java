...
public class InvestmentBriefingScheduler {
  ...
  public void generateDailyBriefing() {
    try {
      ...
      if (!refresh.success()) {
        log.error(
            "오전 투자 브리핑 파이프라인 중단. completedSteps={}, failures={}",
            refresh.completedSteps(),
            refresh.failures());
        return;
      }
      log.info(
          "오전 투자 브리핑 파이프라인 완료. baseDate={}, completedSteps={}",
          refresh.baseDate(),
          refresh.completedSteps());
    } catch (Exception e) {
  ...
  public void generateWeeklyDecision() {
    try {
      ...
      if (!refresh.success()) {
        log.error(
            "주간 투자판단·추천금액 계산 중단. completedSteps={}, failures={}",
            refresh.completedSteps(),
            refresh.failures());
        return;
      }
      log.info(
          "주간 투자판단·추천금액 계산 완료. baseDate={}, completedSteps={}",
          refresh.baseDate(),
          refresh.completedSteps());
    } catch (Exception e) {
      log.error("주간 투자판단·추천금액 계산 실패", e);
    }
  }
