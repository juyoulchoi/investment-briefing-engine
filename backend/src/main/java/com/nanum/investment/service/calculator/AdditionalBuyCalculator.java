...
public class AdditionalBuyCalculator {
    public BigDecimal calculate(BigDecimal stockReturnRate, MarketPhase marketPhase,
            WeightStatus weightStatus, RiskLevel riskLevel, boolean paused,
            boolean fundamentalDamaged, BigDecimal baseAmount, BigDecimal availableCash) {
        if (paused || fundamentalDamaged || weightStatus != WeightStatus.UNDER
                || riskLevel == RiskLevel.VERY_HIGH || baseAmount == null
                || availableCash == null || availableCash.signum() <= 0) return BigDecimal.ZERO;
        BigDecimal recommended = baseAmount.multiply(stockDropAllocation(stockReturnRate)
                .min(marketAllocation(marketPhase))).setScale(0, RoundingMode.DOWN);
