...
import com.nanum.investment.domain.WeekDayCode;
...
public class RegularBuyCalculator {
    public RegularBuySignal calculate(LocalDate date, boolean marketOpen, String cycle,
            String weekDays, Integer monthDay,
        ...
        if (!marketOpen || !isScheduledDate(date, cycle, weekDays, monthDay)) return RegularBuySignal.NOT_SCHEDULED;
    ...
    private boolean isScheduledDate(LocalDate date, String cycle, String weekDays, Integer monthDay) {
        ...
        return switch (cycle) {
            case "DAILY" -> true;
            case "WEEKLY" -> WeekDayCode.parse(weekDays).stream()
