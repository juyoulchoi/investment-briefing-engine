...
public class InvestmentBriefingServiceImpl implements InvestmentBriefingService {
    ...
    public InvestmentBriefingServiceImpl(TbInvestmentBriefingRepository repository,TbInvBrfRepository briefingRepository,
            TbBrfDtlRepository detailRepository,BriefingAiClient briefingAiClient) {
        this.repository=repository;this.briefingRepository=briefingRepository;
        this.detailRepository=detailRepository;this.briefingAiClient=briefingAiClient;
    }
    ...
    public Long generateAndSave(BriefingType briefingType) {
        ...
        detailRepository.saveAll(allItems.stream().map(item->TbBrfDtl.builder()
                .brfId(briefing.getBrfId()).itemCd(item.itemCode()).itemSum(item.summary())
                .itemCont(item.content()).signalCd(item.signalCode())
                .actYn(item.actionRequired()?"Y":"N").build()).toList());
    ...
    private void validate(InvestmentBriefingResponse response){
        if(response==null||response.briefingDate()==null||!StringUtils.hasText(response.title()))
        ...
        if(response.items()==null||response.items().size()!=14||response.finalJudgment()==null)
        ...
        if(!ITEM_CODES.equals(actual))throw new IllegalStateException("브리핑 항목 코드 또는 순서가 기준과 다릅니다: "+actual);
        if(!"FINAL_JUDGMENT".equals(response.finalJudgment().itemCode()))
            throw new IllegalStateException("종합판단 항목 코드가 FINAL_JUDGMENT가 아닙니다.");
        List<BriefingItemResponse> all=new ArrayList<>(response.items());all.add(response.finalJudgment());
        for(BriefingItemResponse item:all){
            if(!StringUtils.hasText(item.summary())||!StringUtils.hasText(item.content())||!StringUtils.hasText(item.signalCode()))
                throw new IllegalStateException(item.itemCode()+" 항목에 필수 설명이 없습니다.");
            if(item.summary().length()>1000)throw new IllegalStateException(item.itemCode()+" 요약이 1000자를 초과했습니다.");
    ...
    public List<TbInvestmentBriefing> findAll(){return repository.findAll(Sort.by(Sort.Direction.DESC,"briefingDate"));}
    public Optional<TbInvestmentBriefing> findById(Long briefingId){return repository.findById(briefingId);}
    public Optional<TbInvestmentBriefing> findByDate(LocalDate briefingDate){return repository.findByBriefingDate(briefingDate);}
    @Transactional public TbInvestmentBriefing save(TbInvestmentBriefing briefing){return repository.save(briefing);}
    @Transactional public void delete(Long briefingId){repository.deleteById(briefingId);}
}
