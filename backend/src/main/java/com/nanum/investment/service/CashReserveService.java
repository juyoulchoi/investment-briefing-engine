...
@Service public class CashReserveService {
 ...
 @Transactional public Result saveReducedBuyAmount(Long accountId,String accountType,String stockCode,LocalDate date,BigDecimal amount,String idempotencyKey){return transact(accountId,accountType,stockCode,date,amount,idempotencyKey,true,"REG_BUY_SAVE","정기매수 감액분 적립",null);}
 @Transactional public Result useForAdditionalBuy(Long accountId,LocalDate date,BigDecimal amount,String idempotencyKey,Long referenceId){return transact(accountId,null,null,date,amount,idempotencyKey,false,"ADD_BUY_USE","추가매수 대기현금 사용",referenceId);}
 private Result transact(Long accountId,String regularBuyAccountType,String regularBuyStockCode,LocalDate date,BigDecimal amount,String key,boolean incoming,String type,String reason,Long referenceId){
  ...
  jdbc.sql("INSERT INTO \"TB_CASH_HIS\"(\"ACCT_ID\",\"REG_BUY_ACCT_TP\",\"REG_BUY_STK_CD\",\"TX_DT\",\"CASH_TP\",\"TX_TP\",\"IN_OUT_TP\",\"TX_AMT\",\"BAL_BFR_AMT\",\"BAL_AFT_AMT\",\"IDEMP_KEY\",\"REF_TBL_NM\",\"REF_ID\",\"TX_RSN\",\"CRT_USR_ID\") VALUES(:accountId,:regularBuyAccountType,:regularBuyStockCode,:date,'RESERVE',:type,:direction,:amount,:before,:after,:key,:refTable,:referenceId,:reason,'SYSTEM')")
   .param("accountId",accountId).param("regularBuyAccountType",regularBuyAccountType).param("regularBuyStockCode",regularBuyStockCode).param("date",date).param("type",type).param("direction",incoming?"IN":"OUT").param("amount",amount).param("before",before).param("after",after).param("key",key).param("refTable",incoming?"TB_REG_BUY":"TB_ADD_BUY").param("referenceId",referenceId).param("reason",reason).update();
