...
public class PortfolioDecisionService {
    ...
    public PortfolioDecision decide(InvestmentDecisionRequest req) {
        ...
        return new PortfolioDecision(d, ma, min, rec, reserve, req.existingReservedCash() + reserve, ds,
                weekly(d, ma, ds), monthly(d, req.positions()));
    }
    ...
    private List<String> weekly(LocalDate d, MarketAssessment m, List<StockDecision> ds) {
        ...
        a.addAll(List.of("다음 주 정기매수 금액을 최소금액 배수로 확정합니다.", "목표비중과 종목 구성은 변경하지 않습니다.", "감액분은 추가매수 대기 현금으로 적립합니다."));
        a.add("증액 대상 " + ds.stream().filter(x -> x.multiplier() > 1).count() + "개, 중단 대상 "
                + ds.stream().filter(x -> x.action() == ActionSignal.PAUSE).count() + "개");
    ...
    private List<String> monthly(LocalDate d, List<StockPosition> ps) {
        ...
        ps.stream().filter(StockPosition::isOverweight).forEach(p -> a.add(p.name() + "은 목표비중 120% 초과로 신규매수를 중단합니다."));
        return a;
    }
}
