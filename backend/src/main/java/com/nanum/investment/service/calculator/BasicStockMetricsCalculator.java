...
public class BasicStockMetricsCalculator {
    ...
    public BasicStockMetrics calculate(
        ...
        BigDecimal purchaseAmount = safeQuantity.multiply(safeAveragePrice)
                .setScale(2, RoundingMode.HALF_UP);
        BigDecimal marketValue = safeQuantity.multiply(safeCurrentPrice)
                .setScale(2, RoundingMode.HALF_UP);
        BigDecimal profitAmount = marketValue.subtract(purchaseAmount)
                .setScale(2, RoundingMode.HALF_UP);

        BigDecimal stockReturnRate = BigDecimal.ZERO;
        if (safeAveragePrice.signum() > 0) {
            stockReturnRate = safeCurrentPrice.subtract(safeAveragePrice)
        ...
        if (accountTotalAmount != null && accountTotalAmount.signum() > 0) {
            currentWeight = marketValue
                    .divide(accountTotalAmount, 8, RoundingMode.HALF_UP)
                    .multiply(ONE_HUNDRED)
                    .setScale(4, RoundingMode.HALF_UP);
        }

        BigDecimal weightDifference = currentWeight.subtract(safeTargetWeight)
                .setScale(4, RoundingMode.HALF_UP);
        return new BasicStockMetrics(
                purchaseAmount, marketValue, profitAmount, stockReturnRate,
