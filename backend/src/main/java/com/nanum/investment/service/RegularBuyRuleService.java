...
public class RegularBuyRuleService {
    ...
    public StockDecision decide(StockPosition p, MarketAssessment m) {
        ...
        if (p.isOverweight())
            return build(p, ActionSignal.PAUSE, 0, r, "현재 비중이 목표비중의 120%를 초과했습니다.");
        ...
        if (m.sentiment().phase() == SentimentPhase.PANIC && p.fundamentalScore() >= 70
        ...
        ActionSignal a = x == 0 ? ActionSignal.PAUSE
                : x < 1 ? ActionSignal.REDUCE
                        : x == 1 ? ActionSignal.KEEP_MINIMUM
    ...
    private StockDecision finish(StockPosition p, ActionSignal a, double x, List<String> r) {
        long rec = Math.min(Math.round(p.minimumBuyAmount() * x), p.effectiveMaximumBuyAmount(MAX));
        long reserve = Math.max(0, p.minimumBuyAmount() - rec);
        return new StockDecision(p.account(), p.code(), p.name(), a, x, p.minimumBuyAmount(), rec, reserve,
                List.copyOf(r));
    }
}
