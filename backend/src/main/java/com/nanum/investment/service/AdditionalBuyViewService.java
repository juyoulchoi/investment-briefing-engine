...
public class AdditionalBuyViewService {
 ...
 public AdditionalBuyViewResult latest(){
  LatestDecision latest=jdbc.sql("SELECT d.\"INV_DEC_ID\",d.\"BASE_DT\" FROM \"TB_INV_DEC\" d WHERE d.\"LATEST_YN\"='Y' AND d.\"DATA_STS\" IN ('FRESH','PARTIAL') AND EXISTS (SELECT 1 FROM \"TB_STK_DEC\" sd JOIN \"TB_ADD_BUY\" b ON b.\"STK_DEC_ID\"=sd.\"STK_DEC_ID\" WHERE sd.\"INV_DEC_ID\"=d.\"INV_DEC_ID\") ORDER BY d.\"BASE_DT\" DESC,d.\"CALC_SEQ\" DESC LIMIT 1").query((rs,n)->new LatestDecision(rs.getLong(1),rs.getObject(2,LocalDate.class))).optional().orElse(null);
  ...
  List<AdditionalBuyViewResult.Candidate> candidates=jdbc.sql("""
   ...
   """).param("day",date).param("decisionId",latest.id()).query((rs,n)->new AdditionalBuyViewResult.Candidate(rs.getLong(1),rs.getLong(2),rs.getString(3),rs.getLong(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getObject(8,Integer.class),rs.getBigDecimal(9),rs.getBigDecimal(10),rs.getString(11),rs.getString(12))).list();
