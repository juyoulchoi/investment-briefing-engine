...
public class MarketSentimentService {
    public SentimentAssessment assess(MarketSnapshot s) {
        ...
        if (structural)
            reasons.add("실적·유동성·가격이 함께 훼손되어 구조적 하락 위험이 있습니다.");
        reversal.addAll(List.of("AI 서비스 매출 증가율 개선", "CAPEX 증가율 둔화 또는 투자 회수 기간 단축", "외국인 현물·선물 동반 순매수 전환",
                "장기금리와 원·달러 환율 안정", "자유현금흐름 개선"));
        risk = Math.max(0, Math.min(100, risk));
        int confidence = Math.min(95, 55 + Math.min(20, Math.abs(s.newsFearScore() - 50) / 2)
                + Math.min(15, Math.abs(s.aiCapexFatigueScore() - 50) / 3));
        return new SentimentAssessment(phase(risk), risk, confidence, structural, List.copyOf(reasons),
                List.copyOf(reversal));
    }
    ...
    private SentimentPhase phase(int r) {
        ...
        if (r >= 70)
            return SentimentPhase.FEAR;
        if (r >= 55)
            return SentimentPhase.FATIGUE;
        if (r >= 40)
            return SentimentPhase.NEUTRAL;
        if (r >= 20)
            return SentimentPhase.OPTIMISM;
