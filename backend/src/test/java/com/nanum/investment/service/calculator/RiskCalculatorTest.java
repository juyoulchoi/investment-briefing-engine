...
class RiskCalculatorTest {
    ...
    void classifiesRiskLevelBoundaries() {
        assertThat(calculate(false, false, "0", WeightStatus.NORMAL,
                MarketPhase.NORMAL, false).level()).isEqualTo(RiskLevel.LOW);
        assertThat(calculate(true, false, "0", WeightStatus.NORMAL,
                MarketPhase.NORMAL, true).level()).isEqualTo(RiskLevel.MEDIUM);
        assertThat(calculate(true, true, "0", WeightStatus.NORMAL,
                MarketPhase.NORMAL, false).level()).isEqualTo(RiskLevel.HIGH);
        assertThat(calculate(true, true, "0", WeightStatus.OVER,
                MarketPhase.NORMAL, false).level()).isEqualTo(RiskLevel.VERY_HIGH);
    }

    @Test
    void accumulatesMinusThirtyPercentScores() {
        RiskResult result = calculate(
                false, false, "-30", WeightStatus.NORMAL,
                MarketPhase.NORMAL, false);
    ...
    private RiskResult calculate(
        ...
        return calculator.calculate(new RiskInput(
                individualStock,
                highRiskProduct,
