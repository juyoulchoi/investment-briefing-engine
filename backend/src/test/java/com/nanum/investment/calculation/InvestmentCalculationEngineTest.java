...
class InvestmentCalculationEngineTest {
 ...
 @Test void marketRegimeBoundariesFollowPolicy(){
 ...
 @Test void emergencyOverridesHighScore(){
  assertThat(classify("95",true)).isEqualTo(MarketRegime.CRASH_RISK);
 }

 @Test void marketScoreAlwaysClampedForExtremeInputs(){
  ...
  for(String extreme:List.of("-100000","0","100000")){
   ...
   var result=calculator.calculate(new MarketScoreInput(x,x,x,x,x,x,x,x,x,x,x,x,x,x,bd("100")),rules);
   assertThat(result.totalScore()).isBetween(BigDecimal.ZERO,bd("100"));
  }
 }

 @Test void sentimentScoreAlwaysClamped(){
  MarketSentimentCalculator calculator=new MarketSentimentCalculator();
  var high=calculator.calculate(new MarketSentimentInput(bd("-1000"),bd("-1000"),bd("1000"),bd("1000"),bd("1000"),bd("-1000"),bd("-1000"),bd("-1000"),bd("100")));
  var low=calculator.calculate(new MarketSentimentInput(bd("1000"),bd("1000"),bd("-1000"),bd("-1000"),bd("-1000"),bd("1000"),bd("1000"),bd("1000"),bd("100")));
  assertThat(high.score()).isEqualByComparingTo("100"); assertThat(low.score()).isZero();
 }
 ...
 @Test void accountAssetIncludesCashReserveAndExchangeRate(){
  var result=new AccountAssetCalculator().calculate(bd("100"),bd("50"),List.of(new AccountAssetCalculator.Holding(bd("2"),bd("10"),bd("2"))));
 ...
 private MarketRegime classify(String score,boolean emergency){
  return regimes.classify(new MarketRegimeInput(bd(score),BigDecimal.ZERO,bd("50"),bd("20"),BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,emergency),rules).value();
 }
