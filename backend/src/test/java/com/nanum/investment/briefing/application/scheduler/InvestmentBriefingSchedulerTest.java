...
import com.nanum.investment.briefing.application.WeeklyInvestmentBriefingService;
...
class InvestmentBriefingSchedulerTest {
  ...
  void weeklyDecisionRunsThroughRawDataOnSaturdayAt0820() throws Exception {
    ...
    Scheduled scheduled =
        InvestmentBriefingScheduler.class
            .getMethod("generateWeeklyDecision")
            .getAnnotation(Scheduled.class);
    assertThat(scheduled.cron())
        .isEqualTo("${investment.briefing.weekly-decision-scheduler.cron:0 20 8 * * SAT}");
  ...
  void weeklyBriefingRunsOnlyStepsElevenThroughThirteenOnSundayAt0820() throws Exception {
    ...
    Scheduled scheduled =
        InvestmentBriefingScheduler.class
            .getMethod("generateWeeklyBriefing")
            .getAnnotation(Scheduled.class);
    assertThat(scheduled.cron())
        .isEqualTo("${investment.briefing.weekly-scheduler.cron:0 20 8 * * SUN}");
    assertThat(scheduled.zone()).isEqualTo("${investment.briefing.scheduler.zone:Asia/Seoul}");
  }
}
