...
function App(){const[page,setPage]=useState<Page>(savedPage),[brief,setBrief]=useState("오늘"),[history,setHistory]=useState("일일"),[reb,setReb]=useState("주간"),[toast,setToast]=useState(""),[refreshing,setRefreshing]=useState(false),[holdingVersion,setHoldingVersion]=useState(0);const move=(p:Page)=>{localStorage.setItem(pageStorageKey,p);setPage(p);scrollTo({top:0,behavior:"smooth"})},notify=(m:string)=>{setToast(m);setTimeout(()=>setToast(""),2200)},refreshMarketData=async()=>{if(page!=="holdings"){notify("보유종목 메뉴에서 갱신해 주세요.");return;}setRefreshing(true);try{const response=await fetch("/api/market-data/holdings/refresh",{method:"POST"});const result=await response.json() as {success:boolean;krxReceivedCounts:Record<string,number>;overseasRequestedCount:number;overseasSuccessCount:number;completedSteps:string[];failures:string[]};if(!response.ok)throw new Error(`HTTP ${response.status}`);setHoldingVersion(v=>v+1);const domestic=Object.values(result.krxReceivedCounts||{}).reduce((sum,count)=>sum+count,0);notify(result.success?`1~10단계 ${result.completedSteps.length}개 완료 · 국내·ETF ${domestic.toLocaleString("ko-KR")}건 · 해외 ${result.overseasSuccessCount}/${result.overseasRequestedCount}개`:`일부 갱신 실패: ${result.failures.join(" · ")}`);}catch(e){notify(e instanceof Error?e.message:"시장 데이터 갱신에 실패했습니다.");}finally{setRefreshing(false);}};return <div className="shell"><aside><button className="brand" onClick={()=>move("dashboard")}><b>F</b><span><strong>FINBRIEF</strong><small>Investment OS</small></span></button><nav><p>OVERVIEW</p>{nav.slice(0,1).map(x=><Nav key={x[0]} x={x} active={page===x[0]} go={move}/>)}<p>INVEST</p>{nav.slice(1,5).map(x=><Nav key={x[0]} x={x} active={page===x[0]} go={move}/>)}<p>RECORDS</p>{nav.slice(5).map(x=><Nav key={x[0]} x={x} active={page===x[0]} go={move}/>)}</nav><div className="user"><b>민</b><span><strong>김투자</strong><small>개인 포트폴리오</small></span></div></aside><main><header className="top"><div><h1>{nav.find(x=>x[0]===page)?.[1]}</h1><p>2026년 7월 30일 목요일 · 장 마감 기준</p></div><div><span className="latest">● 데이터 최신</span><button className="primary" disabled={refreshing} onClick={refreshMarketData}>{refreshing?"갱신 중...":"↻ 브리핑 갱신"}</button></div></header>{page==="dashboard"&&<Dashboard go={move}/>} {page==="briefing"&&<Briefing period={brief} set={setBrief}/>} {page==="holdings"&&<Holdings key={holdingVersion}/> } {page==="additional"&&<Additional/>} {page==="rebalance"&&<Rebalance period={reb} set={setReb}/>} {page==="history"&&<History period={history} set={setHistory}/>} {page==="reference"&&<ReferenceAdmin notify={notify}/>} {page==="operations"&&<OperationsAdmin notify={notify}/>} {page==="marketadmin"&&<MarketAnalysisAdmin notify={notify}/>} {page==="bondyields"&&<BondYieldPage notify={notify}/>}</main><nav className="mobile">{nav.slice(0,5).map(x=><button key={x[0]} className={page===x[0]?"active":""} onClick={()=>move(x[0])}><b>{x[2]}</b>{x[1]}</button>)}</nav>{toast&&<div className="toast">{toast}</div>}</div>}
...
function Additional(){type Candidate={additionalBuyId:number;stockCode:string;stockName:string;priority:number|null;score:number;recommendedAmount:number;reason:string;executedYn:string};type Result={baseDate:string|null;reserveAmount:number;recommendedTotal:number;usageRate:number;candidates:Candidate[]};const[data,setData]=useState<Result|null>(null),[error,setError]=useState("");useEffect(()=>{fetch("/api/investment/buy-plans/additional/latest").then(async r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json() as Promise<Result>}).then(setData).catch(e=>setError(e instanceof Error?e.message:"추가매수 추천을 불러오지 못했습니다."))},[]);if(error)return <div className="page"><section className="card empty"><p>{error}</p></section></div>;if(!data)return <div className="page"><section className="card empty"><p>추가매수 추천을 불러오는 중입니다.</p></section></div>;return <div className="page"><section className="cash"><div><small>추가매수 확보현금</small><strong>{won(data.reserveAmount)}</strong><p>{data.baseDate?`${data.baseDate} 계산 기준`:"계산된 추가매수 계획 없음"} · 최대 사용 권장액 {won(data.recommendedTotal)}</p></div><div><i><b style={{width:`${Math.min(100,data.usageRate)}%`}}/></i><span>확보현금 사용 권장률 <strong>{data.usageRate.toFixed(1)}%</strong></span></div></section><section className="card recommends"><Head k="OPPORTUNITY" t="추천종목" a="우선순위 순"/>{data.candidates.length===0?<p className="data-state">현재 추가매수 조건을 충족한 종목이 없습니다.</p>:data.candidates.map((r,i)=><article key={r.additionalBuyId}><b>{String(r.priority??i+1).padStart(2,"0")}</b><span><strong>{r.stockName||r.stockCode}</strong><small>{r.stockCode} · {r.reason}</small></span><em>추천점수 {Number(r.score).toFixed(0)}</em><strong>{won(Number(r.recommendedAmount))}</strong></article>)}</section></div>}
function Rebalance({period,set}:{period:string,set:(s:string)=>void}){type Item={rebalanceItemId:number;accountType:string;stockCode:string;stockName:string;currentAmount:number;targetAmount:number;currentWeight:number;targetWeight:number;currentRegularBuyAmount:number;newRegularBuyAmount:number;changeAmount:number;action:string;reason:string};type Result={baseDate:string|null;type:string;items:Item[]};const[data,setData]=useState<Result|null>(null),[error,setError]=useState("");useEffect(()=>{setData(null);setError("");fetch(`/api/investment/rebalancing/latest?type=${period==="주간"?"WEEKLY":"MONTHLY"}`).then(async r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json() as Promise<Result>}).then(setData).catch(e=>setError(e instanceof Error?e.message:"리밸런싱 계산안을 불러오지 못했습니다."))},[period]);const rows=data?.items.filter(x=>x.action!=="HOLD")||[];return <div className="page"><Tabs vals={["주간","월간"]} active={period} set={set}/><section className="card rebalance"><Head k={period==="주간"?"AMOUNT REBALANCE":"WEIGHT REBALANCE"} t={period==="주간"?"정기매수 금액 변경":"목표비중 리밸런싱"} a={`${rows.length}개 조정`}/><p>{data?.baseDate?`${data.baseDate} 자동 계산 결과`:"계산된 리밸런싱 계획이 없습니다."}</p>{error?<p className="data-state">{error}</p>:!data?<p className="data-state">불러오는 중입니다.</p>:rows.length===0?<p className="data-state">현재 조정이 필요한 종목이 없습니다.</p>:rows.map(r=>{const before=period==="주간"?won(Number(r.currentRegularBuyAmount)):`${Number(r.currentWeight).toFixed(1)}%`,after=period==="주간"?won(Number(r.newRegularBuyAmount)):`${Number(r.targetWeight).toFixed(1)}%`,change=period==="주간"?`${Number(r.changeAmount)>=0?"+":""}${won(Number(r.changeAmount))}`:`${Number(r.targetWeight-r.currentWeight)>=0?"+":""}${Number(r.targetWeight-r.currentWeight).toFixed(1)}%p`;return <article key={r.rebalanceItemId}><strong>{r.stockName}<small>{r.accountType} · {r.stockCode}</small></strong><span>{before}</span><b>→</b><span>{after}</span><em className={Number(r.changeAmount)>0?"pos":Number(r.changeAmount)<0?"neg":""}>{change}</em></article>})}</section></div>}
