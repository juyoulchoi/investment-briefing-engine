...
function App(){const[page,setPage]=useState<Page>(savedPage),[brief,setBrief]=useState("오늘"),[history,setHistory]=useState("일일"),[reb,setReb]=useState("주간"),[toast,setToast]=useState(""),[refreshing,setRefreshing]=useState(false),[holdingVersion,setHoldingVersion]=useState(0);const move=(p:Page)=>{localStorage.setItem(pageStorageKey,p);setPage(p);scrollTo({top:0,behavior:"smooth"})},notify=(m:string)=>{setToast(m);setTimeout(()=>setToast(""),2200)},refreshMarketData=async()=>{if(page!=="holdings"){notify("보유종목 메뉴에서 갱신해 주세요.");return;}setRefreshing(true);try{const response=await fetch("/api/market-data/holdings/refresh",{method:"POST"});const result=await response.json() as {success:boolean;krxReceivedCounts:Record<string,number>;overseasRequestedCount:number;overseasSuccessCount:number;failures:string[]};if(!response.ok)throw new Error(`HTTP ${response.status}`);setHoldingVersion(v=>v+1);const domestic=Object.values(result.krxReceivedCounts||{}).reduce((sum,count)=>sum+count,0);notify(result.success?`국내·ETF ${domestic.toLocaleString("ko-KR")}건, 해외 ${result.overseasSuccessCount}/${result.overseasRequestedCount}개 갱신 완료`:`일부 갱신 실패: ${result.failures.join(" · ")}`);}catch(e){notify(e instanceof Error?e.message:"시장 데이터 갱신에 실패했습니다.");}finally{setRefreshing(false);}};return <div className="shell"><aside><button className="brand" onClick={()=>move("dashboard")}><b>F</b><span><strong>FINBRIEF</strong><small>Investment OS</small></span></button><nav><p>OVERVIEW</p>{nav.slice(0,1).map(x=><Nav key={x[0]} x={x} active={page===x[0]} go={move}/>)}<p>INVEST</p>{nav.slice(1,5).map(x=><Nav key={x[0]} x={x} active={page===x[0]} go={move}/>)}<p>RECORDS</p>{nav.slice(5).map(x=><Nav key={x[0]} x={x} active={page===x[0]} go={move}/>)}</nav><div className="user"><b>민</b><span><strong>김투자</strong><small>개인 포트폴리오</small></span></div></aside><main><header className="top"><div><h1>{nav.find(x=>x[0]===page)?.[1]}</h1><p>2026년 7월 30일 목요일 · 장 마감 기준</p></div><div><span className="latest">● 데이터 최신</span><button className="primary" disabled={refreshing} onClick={refreshMarketData}>{refreshing?"갱신 중...":"↻ 브리핑 갱신"}</button></div></header>{page==="dashboard"&&<Dashboard go={move}/>} {page==="briefing"&&<Briefing period={brief} set={setBrief}/>} {page==="holdings"&&<Holdings key={holdingVersion}/> } {page==="additional"&&<Additional notify={notify}/>} {page==="rebalance"&&<Rebalance period={reb} set={setReb} notify={notify}/>} {page==="history"&&<History period={history} set={setHistory}/>} {page==="reference"&&<ReferenceAdmin notify={notify}/>} {page==="operations"&&<OperationsAdmin notify={notify}/>} {page==="marketadmin"&&<MarketAnalysisAdmin notify={notify}/>} {page==="bondyields"&&<BondYieldPage notify={notify}/>}</main><nav className="mobile">{nav.slice(0,5).map(x=><button key={x[0]} className={page===x[0]?"active":""} onClick={()=>move(x[0])}><b>{x[2]}</b>{x[1]}</button>)}</nav>{toast&&<div className="toast">{toast}</div>}</div>}
