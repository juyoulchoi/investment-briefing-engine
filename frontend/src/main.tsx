...
function Additional(){type Candidate={additionalBuyId:number;stockCode:string;stockName:string;eligibleYn:string;priority:number|null;score:number;recommendedAmount:number;reason:string;executedYn:string};type Result={baseDate:string|null;reserveAmount:number;recommendedTotal:number;usageRate:number;candidates:Candidate[]};const[data,setData]=useState<Result|null>(null),[error,setError]=useState("");useEffect(()=>{fetch("/api/investment/buy-plans/additional/latest").then(async r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json() as Promise<Result>}).then(setData).catch(e=>setError(e instanceof Error?e.message:"추가매수 추천을 불러오지 못했습니다."))},[]);if(error)return <div className="page"><section className="card empty"><p>{error}</p></section></div>;if(!data)return <div className="page"><section className="card empty"><p>추가매수 추천을 불러오는 중입니다.</p></section></div>;const eligibleCount=data.candidates.filter(r=>r.eligibleYn==="Y").length;return <div className="page"><section className="cash"><div><small>추가매수 확보현금</small><strong>{won(data.reserveAmount)}</strong><p>{data.baseDate?`${data.baseDate} 계산 기준`:"계산된 추가매수 계획 없음"} · 최대 사용 권장액 {won(data.recommendedTotal)}</p></div><div><i><b style={{width:`${Math.min(100,data.usageRate)}%`}}/></i><span>확보현금 사용 권장률 <strong>{data.usageRate.toFixed(1)}%</strong></span></div></section><section className="card recommends"><Head k="OPPORTUNITY" t="추가매수 평가종목" a={eligibleCount + "개 적격 · 전체 " + data.candidates.length + "개"}/>{data.candidates.length===0?<p className="data-state">계산된 추가매수 평가 결과가 없습니다.</p>:data.candidates.map((r,i)=><article key={r.additionalBuyId} className={r.eligibleYn==="Y"?"":"muted-row"}><b>{r.eligibleYn==="Y"?String(r.priority??i+1).padStart(2,"0"):"-"}</b><span><strong>{r.stockName||r.stockCode}</strong><small>{r.stockCode} · {r.reason||"추가매수 조건 미충족"}</small></span><em>{r.eligibleYn==="Y"?"추천점수 "+Number(r.score).toFixed(0):"제외"}</em><strong>{won(Number(r.recommendedAmount))}</strong></article>)}</section></div>}
function Rebalance({period,set}:{period:string,set:(s:string)=>void}){type Item={rebalanceItemId:number;accountType:string;stockCode:string;stockName:string;currentAmount:number;targetAmount:number;currentWeight:number;targetWeight:number;currentRegularBuyAmount:number;newRegularBuyAmount:number;changeAmount:number;action:string;reason:string};type Result={baseDate:string|null;type:string;items:Item[]};const[data,setData]=useState<Result|null>(null),[error,setError]=useState("");useEffect(()=>{setData(null);setError("");fetch(`/api/investment/rebalancing/latest?type=${period==="주간"?"WEEKLY":"MONTHLY"}`).then(async r=>{if(!r.ok)throw new Error(`HTTP ${r.status}`);return r.json() as Promise<Result>}).then(setData).catch(e=>setError(e instanceof Error?e.message:"리밸런싱 계산안을 불러오지 못했습니다."))},[period]);const rows=data?.items||[],adjustmentCount=rows.filter(x=>x.action!=="HOLD").length;return <div className="page"><Tabs vals={["주간","월간"]} active={period} set={set}/><section className="card rebalance"><Head k={period==="주간"?"AMOUNT REBALANCE":"WEIGHT REBALANCE"} t={period==="주간"?"정기매수 금액 변경":"목표비중 리밸런싱"} a={`${adjustmentCount}개 조정 · 전체 ${rows.length}개`}/><p>{data?.baseDate?`${data.baseDate} 자동 계산 결과`:"계산된 리밸런싱 계획이 없습니다."}</p>{error?<p className="data-state">{error}</p>:!data?<p className="data-state">불러오는 중입니다.</p>:rows.length===0?<p className="data-state">계산된 리밸런싱 평가 결과가 없습니다.</p>:rows.map(r=>{const before=period==="주간"?won(Number(r.currentRegularBuyAmount)):`${Number(r.currentWeight).toFixed(1)}%`,after=period==="주간"?won(Number(r.newRegularBuyAmount)):`${Number(r.targetWeight).toFixed(1)}%`,change=period==="주간"?`${Number(r.changeAmount)>=0?"+":""}${won(Number(r.changeAmount))}`:`${Number(r.targetWeight-r.currentWeight)>=0?"+":""}${Number(r.targetWeight-r.currentWeight).toFixed(1)}%p`;return <article key={r.rebalanceItemId} className={r.action==="HOLD"?"muted-row":""}><strong>{r.stockName}<small>{r.accountType} · {r.stockCode} · {r.action==="HOLD"?"유지":r.reason}</small></strong><span>{before}</span><b>→</b><span>{after}</span><em className={Number(r.changeAmount)>0?"pos":Number(r.changeAmount)<0?"neg":""}>{r.action==="HOLD"?"유지":change}</em></article>})}</section></div>}
