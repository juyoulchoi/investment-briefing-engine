...
function Holdings(){
 ...
 const[holdings,setHoldings]=useState<Holding[]>([]),[selected,setSelected]=useState<Account["accountType"]>("DOMESTIC"),[sort,setSort]=useState("profitAsc"),[loading,setLoading]=useState(true),[error,setError]=useState(""),[editMode,setEditMode]=useState(false),[drafts,setDrafts]=useState<Record<number,Draft>>({}),[saving,setSaving]=useState(false),[saveError,setSaveError]=useState("");
 ...
 const rows=holdings.filter(h=>h.accountType===selected).sort((a,b)=>sort==="profitAsc"?Number(a.profitLossRate)-Number(b.profitLossRate):sort==="profitDesc"?Number(b.profitLossRate)-Number(a.profitLossRate):Number(b.evaluationAmount)-Number(a.evaluationAmount)),evaluation=rows.reduce((sum,h)=>sum+Number(h.evaluationAmount||0),0),cost=rows.reduce((sum,h)=>sum+Number(h.averagePrice||0)*Number(h.holdingQuantity||0),0),profit=evaluation-cost;
 ...
 const selectAccount=(type:Account["accountType"])=>{cancelEdit();setSelected(type);};
